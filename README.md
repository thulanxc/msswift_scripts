...
| `start_ray.sh` | 启动 Ray 集群。自动探测有 IP 的网络接口和 IB 设备，保存到 `.nccl_env` |
| `train_config.yaml` | 训练参数配置。需修改 `dataset` 路径 |
| `train_ray.sh` | 在 head 节点提交训练任务。自动注入 NCCL 环境变量到 Ray actor |
...
| 文件 | 说明 |
|------|------|
| `check_network.sh` | 网络诊断工具。区分有/无 IP 接口，自动推荐 NCCL_SOCKET_IFNAME |
...
# ---- 在 head 节点 (选一台) ----
bash start_ray.sh head                     # 启动 Ray head (自动探测 NCCL 网络)
...
bash train_ray.sh                          # 启动训练 (自动注入 NCCL 环境变量)
```

> **NCCL 报错？** 运行 `bash check_network.sh` 查看推荐接口，编辑 `.nccl_env` 修改 `NCCL_SOCKET_IFNAME` 后重新 `bash train_ray.sh`。
