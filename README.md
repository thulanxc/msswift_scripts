...
| 文件 | 说明 |
|------|------|
| `nodes.conf` | **唯一需要手动编辑的文件**。填入节点 IP (有几台写几行)、SSH 用户名、数据/模型路径 |
...
| `train_multinode.sh` | 核心多机训练脚本。接受 `NODE_RANK` 参数，由 `launch_all.sh` 自动调用 |
...
```bash
vim nodes.conf          # 填节点 IP (有几台写几行) + 数据路径
...
节点数量灵活可变——在 `nodes.conf` 的 `NODES` 数组中增减 IP 行即可，其他脚本自动适配。
